...
class reader:
    ...
    def __init__(self, filename, needFresh=True, language='chn'):
        ...
        with open(filename, "r", encoding='utf-8') as f:
            self.raw_text = f.readlines()[:2]
        print(len(self.raw_text))
        if needFresh:
            if language == 'chn':
                text = refresh_chn_data(self.raw_text)
                self.text_list = text.getText()
            else:
                text = refresh_eng_data(self.raw_text)
                self.text_list = text.getText()
        else:
            self.text_list = [line.strip() for line in self.raw_text]
        # print(self.text_list[:1])
        for i in range(len(self.text_list)):
            self.text_list[i] = self.text_list[i].split(' ')
